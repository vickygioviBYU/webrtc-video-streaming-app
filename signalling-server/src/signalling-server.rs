...
// Generating an id for the session
...
// The message goes to one peer, or to another?
...
fn handle_message(
    ...
    let (message_to_client, destination) = match result {
        ...
        // Send Ice Candidates from one peer to another
...
// Raw stream (Streaming, with incoming and outcoming, that is a bytes stream)
...
async fn handle_connection(
    ...
    // Keep active the connection, and wait
    future::select(broadcast_incoming, receive_from_others).await;

    // Disconnect by deleting the address from the peer map, and the user from the user list
